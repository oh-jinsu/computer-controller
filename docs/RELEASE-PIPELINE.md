...
공개 베타는 `bash Mac-Release.command --publish-prerelease`로 게시합니다. 정식 latest로 지정하지 않으며,
Developer ID·공증·파일 해시·대응 소스 검증은 동일하게 적용합니다. 공개 전환은 별도의 소유자 요청으로만 수행합니다.

빌더는 최초 배포 시 대응 소스와 저작권 고지를 수집하고 캐시합니다. 다음 버전은 검증된 캐시를 재사용합니다.
앱에 기록한 소스 아카이브 해시와 게시할 소스가 다르면 배포하지 않습니다. 사용자 설치에는 이 과정이 없습니다.
