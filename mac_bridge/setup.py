...
def verify_installation(root: Path, *, force: bool = False) -> None:
    ...
    for key in ('CONTROL_PLANE_API_KEY', 'OPENAI_API_KEY', 'OPENAI_ADMIN_KEY'):
        env.pop(key, None)
    for script, timeout in [('tests/smoke_mac_mcp.py', 180), ('tests/smoke_mcp.py', 120), ('tests/smoke_approval_mcp.py', 180), ('tests/smoke_browser_mcp.py', 180), ('tests/smoke_request_logging.py', 180)]:
