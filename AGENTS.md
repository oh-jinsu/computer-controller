...
- Retain local per-request approval for writes/terminal/input. Do not add an MCP argument or environment variable that silently approves remote requests.
...
- Run `python -m unittest discover -s tests -p 'test_*.py' -v`. Real MCP smoke tests are separate processes (`tests/smoke_mac_mcp.py`, `tests/smoke_mcp.py`) and need installed dependencies.
