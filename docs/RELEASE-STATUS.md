...
## 로컬 서버 교체용 0.5.0-beta.7

앱 시작 시 화면 캡처 권한 확인·최초 승인 요청을 추가한 로컬 실행본을 준비합니다. 파일명은 `Mac-Bridge-0.5.0-beta.7-macos26-arm64.zip`입니다. 아직 이 버전의 공증·공개 배포를 완료한 것은 아니며, 위 beta.2의 공증 결과를 이 버전에 적용하지 않습니다.

## 첫 공개판

`0.5.0-beta.7`은 사용자 요청에 따라 공개 베타로 배포합니다. 앱 기능은 beta.6과 동일하며 배포 고지·해당 버전의 종속성 소스를 보완합니다. 별도 Chrome 작업 창·파일 업로드는 아직 포함하지 않습니다. 최신 안정판 피드의 자동 업데이트 및 다른 Mac에서의 최초 연결은 추가 검증 대상입니다. 실제 서명·공증·다운로드 검사 결과는 해당 릴리스의 검증 파일을 확인하세요.
