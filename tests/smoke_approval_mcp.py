...
async def test():
    ...
    with tempfile.TemporaryDirectory(prefix='mac-approval-smoke-') as tmp:
        ...
        async with connect(root) as client:
            ...
            r = await client.call_tool('edit_file', {'path': 'sample.txt', 'old_string': 'after', 'new_string': 'edited'})
            assert not r.is_error, r
            assert p.read_text() == 'edited\n'
            r = await client.call_tool('create_directory', {'path': 'nested/path'})
            assert not r.is_error, r
            assert (project / 'nested/path').is_dir()
            (project / 'move-me.txt').write_text('move marker')
            r = await client.call_tool('move_file', {'source': 'move-me.txt', 'destination': 'nested/path/moved.txt'})
            assert not r.is_error, r
        ...
        async with connect(root) as client:
            ...
            r = await client.call_tool('write_file', {'path': 'restart.txt', 'content': 'persisted'})
            assert not r.is_error, r
            ...
            r = await client.call_tool('pause', {})
            assert not r.is_error, r
            r = await client.call_tool('start_process', {'command': 'printf MUST_NOT_RUN'})
            assert r.is_error, r
