...
앱 시작 시 화면 캡처 권한 확인·최초 승인 요청을 추가한 로컬 실행본을 준비합니다. 파일명은 `Mac-Bridge-0.5.0-beta.6-macos26-arm64.zip`입니다. 아직 이 버전의 공증·공개 배포를 완료한 것은 아니며, 위 beta.2의 공증 결과를 이 버전에 적용하지 않습니다.
