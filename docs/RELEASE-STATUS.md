...
`0.5.0-beta.8`은 공개 베타로 배포합니다. 기존 사용자의 저장된 승인·브라우저 모드는 유지하며, 새 설치는 `always`와 평소 Chrome(`personal`)이 기본입니다. 별도 Chrome 작업 창·파일 업로드는 아직 포함하지 않습니다. 실제 서명·공증·다운로드 검사 결과는 해당 릴리스의 검증 파일을 확인하세요.
