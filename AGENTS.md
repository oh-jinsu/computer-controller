...
- The npm CLI is a bootstrap/host for the same Computer Controller core, not a second MCP implementation. Keep `package.json` and `packaging/release.json` versions aligned, pin/verify downloaded tunnel-client archives, never put runtime/API keys in command arguments or npm logs, and keep GUI/CLI on the same per-machine launch lock. npm Registry publication is a separate explicit external action; local `npm pack` or GitHub-source installation is not a registry publish.
