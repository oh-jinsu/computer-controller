...
현재는 내부 시험판이며 저장소 공개 전환, Developer ID Application 인증서·Apple 공증, 공개 주소에서 실제 앱 업데이트 검증은 별도로 남아 있습니다. 실행 중인 터미널 서버는 자동으로 교체하지 않았습니다. 아래의 이전 버전 설명은 기존 사용자·개발자용 이력입니다.

[현재 앱 배포·업데이트 구조와 검증 범위](docs/APP-DISTRIBUTION.md)
