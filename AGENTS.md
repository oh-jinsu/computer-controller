...
- Run `python -m unittest discover -s tests -p 'test_*.py' -v`. Real MCP smoke tests are separate processes (`tests/smoke_mac_mcp.py`, `tests/smoke_mcp.py`, `tests/smoke_approval_mcp.py`) and need installed dependencies.
